...
export function HashGenerator() {
  ...
  const generate = async (text: string) => {
    ...
    setResults(ALGORITHMS.map(a => ({ algorithm: a, value: '', loading: true })))
    const computed = await Promise.all(
      ALGORITHMS.map(async (alg) => {
        if (alg === 'MD5')
          return { algorithm: alg, value: md5(text), loading: false }
          ...
          <div key={algorithm} className="flex items-center justify-between px-4 py-3 rounded-lg border border-border">
            <div className="flex-1 min-w-0">
              ...
              <Typography className="font-mono text-sm truncate text-foreground/80">
                {loading ? '...' : (value || <span className="text-muted-foreground">—</span>)}
              </Typography>
            </div>
            <Button
              size="sm"
